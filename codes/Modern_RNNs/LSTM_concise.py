...
def init_state(num_layers, batch_size, hidden_size, device):
    hidden_state = torch.zeros((num_layers, batch_size, hidden_size), device=device)
    memory_state = torch.zeros((num_layers, batch_size, hidden_size), device=device)
...
def main(batch_size=32, epochs=500, lr=1.0, num_steps=32, hidden_size=256):
    ...
    gru = nn.LSTM(vocab_size, hidden_size)
    net = RNN_Moduel(gru, init_state, vocab_size, hidden_size, device).to(device)
    ...
    trainer(epochs, data_iter, vocab, 
            ...
            need_to_clip=True, fig_name='lstm_concise', need_to_predict=True)
